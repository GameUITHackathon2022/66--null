...
export var disaster_type: = DISASTER.FLOOD
...
func _ready():
	match(disaster_type):
		DISASTER.FLOOD:
			...
			conditions_value.append(100)
			conditions.append(TYPE.DEF)
			conditions_value.append(100)
		...
		DISASTER.DROUGHT:
			conditions.append(TYPE.WATER)
			conditions_value.append(100)
			conditions.append(TYPE.LAND)
			conditions_value.append(100)
			
			nameLabel.text = "Hạn hán"
			disaster.texture = drought
			icon1.texture = land
			icon2.texture = water
			maxValue = 20
			setValue1(0)
...
func checkIfWin():
	if value1 == maxValue and value2 == maxValue:
		get_parent().get_parent().win.show()
		get_parent().get_parent().isWin = true
